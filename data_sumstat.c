...
void set_almap(int* almap, char** list, int n, int segsites, int start, int end){
  ...
  int i,j;
  for(i=0; i<segsites; i++){ // for each seg site
    int ones = 0;
    
    for( j=start; j<end; j++) // count the number of alt alleles, I think
      ones += ( list[j][i] == '1' );
...
int thomsonEst(double* thomson, int* almap, int n, int segsites, int* missing){
  ...
  for(i=0; i<segsites; ++i){ // for each polymorphism on the locus
    nsam = n - missing[i]; // calculate number of covered samples
    
    if(nsam <= 0) return 0;
    
    if(almap[i] == 0 || almap[i] == nsam) continue;

    te += ((double)almap[i]/(double)nsam); // the statistic is calculated in a single line
                                           // almap is number of alt alleles by site
                                           // nsam is the number of samples
                                           // so it is simply the number of alt alleles per sample
  }
  *thomson = te;
