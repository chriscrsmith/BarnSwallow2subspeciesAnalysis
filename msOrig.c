...
int usage3(){
  ...
  fprintf(stderr, "-Z <events> <min> <max> : log-transformed confitional uniform (different from log uniform). Draws log(U(min, max)) and rescales from 0 to 1, then multiplies by the specified event time\n");
...
double get_param(int arg, char distr){
  ...
  case 'R':
    ...
    break;
  case 'C':
    // I want argument one below to be the min of the prior range we want
    // and the second argument to be another event time prior 
    eventNum = param_distr_values[arg][0] - 1; // minus 1 because index 0. (this is reading the number you specifiy after -C )
    ...
    return (uniform_ab(0 , 1) * timeevents[eventNum]);
    break;
  case 'D':
  ...
  case 'Z':
    ...
    if( event <= eventNum ){
      printf("\n ERROR: \n");
      printf(" You have asked for a conditional uniform prior (-Z) but \n");
      printf(" specified a conditional event <= the current event. \n");
      printf(" The conditional event needs to come before the current event in your command. \n\n");
    }
    return (logTransUniform(param_distr_values[arg][1], param_distr_values[arg][2]) * timeevents[eventNum]) ;
    break;
  case 'S':
