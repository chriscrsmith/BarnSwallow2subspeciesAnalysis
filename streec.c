...
double currentPop1Size, currentPop2Size; // chris
...
/* Initialization */
	if( chrom == NULL ) {
	  ...
	  /* Added by JRI Aug. 2007.  Prevents crashing if new sample size from tbs is 20 or more greater than original sample size */
	if( nsam > maxchr-20 ){
		maxchr = nsam + 20;
		chrom = (struct chromo *)realloc( chrom, (unsigned)(maxchr*sizeof(struct chromo))) ;
		if( chrom == NULL ) perror( "realloc error. segtre");	
	}  
	if( nnodes == NULL ){
	...
	for(pop=ind=0;pop<npop;pop++){
	  //fprintf(stderr, "inconfig[pop]: %d\n", inconfig[pop]);
	  for(j=0; j<inconfig[pop];j++,ind++) {
	    //fprintf(stderr, "ind: %d\n", ind);
	    chrom[ind].nseg = 1;
	    if( !(chrom[ind].pseg = malloc((unsigned)sizeof(struct seg)) ))
	...
	seglst[0].beg = 0;
	if( !(seglst[0].ptree = (struct node *)calloc((int)(2*nsam),sizeof(struct node)) ))
...
/* Main loop */
	...
	if( ( ( eflag == 0) && (nextevent != NULL))|| ( (nextevent != NULL) &&  ( (t+tmin) >=  nextevent->time)) ) {
		...
		case 'n' :
		   ...
		   // *** chris: comment above line, and uncomment below three lines, to add pop2 size to pop1 at split
		   //currentPop1Size = size[0] * exp(-alphag[0]*t);
		   //currentPop2Size = size[1] * exp(-alphag[1]*t);
		   //size[0] = (currentPop1Size + currentPop2Size); 
		   // ***
		   // *** another option: comment the above line, and uncomment the below, in order to multiply the current size by x
		   //currentPop1Size = size[0] * exp(-alphag[0]*t);
		   //size[0] = (currentPop1Size * nextevent->paramv);
		   // ***
		...
		case 'g' :
		     pop = nextevent->popi ;
		     // dude. 
		     // I think this is printing the pop size...
		     // and if you change the pop size with a -en command, it changes the pop to that size, instead of multiplying it by that size like the manual says...
		     // I'm doing a quick simulation experiment to test this...
		     size[pop] = size[pop]*exp( - alphag[pop]*(t-tlast[pop]) ) ;
		     alphag[pop]= nextevent->paramv ;
		     //printf("\n\nalphag: %f\n\n", alphag[pop]);
		     tlast[pop] = t ;
		...
		case 'M' :
		   ...
		   for( pop = 0; pop <npop; pop++)
		     //{ // chris added this bracket to add a print statement
		     //printf("\n\nmig: %f\n", nextevent->paramv);
		       migm[pop][pop]= nextevent->paramv ; // this is indeed the migration rate specified on the command line
		       //printf("\n\nmig: %f", migm[pop][pop]);
		       //printf("\nsize: %f\n", size[pop]);
		       //} // chris added this
		   nextevent = nextevent->nextde ;
	...
	int start, c, *psg;
{
	...
	ns = chrom[c].nseg;
	pseg = chrom[c].pseg;
	/* fprintf(stderr, "psg: %d\n", *psg); */
/* 	fprintf(stderr, "ns: %d\n", ns); */
/* 	fprintf(stderr, "start: %d\n", start); */
/* 	fprintf(stderr, "pseg: %d\n", (pseg)->beg); */
/* 	fprintf(stderr, "pseg+psg: %d\n", (pseg+(*psg))->beg); */
