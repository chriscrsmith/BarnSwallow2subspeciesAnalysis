...
int eventNum; // keeping track of event numbers for -C an -S
double priorLimit; // event time prior minimum/or maximum for -C
double priorLimit1; // event time prior minimum/or maximum for -Z
double priorLimit2;// event time prior minimum/or maximum for -Z
double foldChange; // fold change in pop size for -A
int event1; 
int event2;
int timeElapsed;
int ct1, ct2;
int specifiedTime; 
double alfa; // chris's alfa
...
*/
char distros[NDIST] = {'U', 'N', 'G', 'L', 'R', 'C', 'Y', 'Z', 'S', 'A', 'B', 'D'};	
		...
		*/
		...
		if(truesegsites > 0 &&
		  ...
		  calculations(weights, 
		   ...
		   // this is chris's fix                                                                        
                   // if the number of seg sits for a locus == 0, fst is not calculated (see above if statement)
...
getpars(int argc, char *argv[], int *phowmany )
{
  ...
  while( arg < argc ){
    ...
    int c;
    if(!count)
      //fprintf(stdout, "arg %i argv[arg] %s argc[arg] %i \n", arg, argv[arg], argc);
...
/* 	    } */
	    ...
	    // chris's block of code
	    else if(argv[arg][1] == 's'){
	      int ppp1 = (argv[arg][2] - '0') - 1; // this is for converting a char to an integer, and -1 for index
	      int ppp2 = (argv[arg][3] - '0') - 1;
	      pars.cp.mig_mat[pop][pop2] = pars.cp.mig_mat[ppp1][ppp2];
	      global_values[arg] = pars.cp.mig_mat[pop][pop2]; // don't know what global values are for
	      arg = arg+1; // iterating
	    }
	    // 
	    else if( count && param_distr[arg - 1] != '-'){
	      ...
	      global_values[arg] = pars.cp.mig_mat[pop][pop2]; 
	      arg = arg + distribution_arguments( param_distr[arg-1] ) + 1;
	    }
	    else if( arg < argc && argv[arg][0] != '-'){
	      argcheck( arg, argc, argv);
	...
	*/
	...
	pt->popi = atoi( argv[arg++] ) -1 ;
	int zing;
	//	for( zing =0; zing < 50; zing++){
	//printf("\nvar %i: %s %f \n", zing, argv[zing], atof(argv[zing]));
	//}
	//printf("\n\npt: %f\n\n", atoi( argv[arg] ));
	if( arg >= argc ) { fprintf(stderr,"Not enough arg's after -eg.\n"); usage(); }
...
int getmissing(FILE* missingin, int*** misseq, double*** misstart, double*** misend, int** missingdata){
  ...
  int i = 0, j = 0, jj=0, countingFiles=0;
  int limx=1000, limy=100;
  //int m1;
  //double m2,m3; 
  struct rec // chris change
  {
    int m1;
    double m2,m3;
  };
  ...
  *missingdata = malloc((unsigned)limy * sizeof(int));
  ...
  while(fscanf(missingin, "%s", filename)!=EOF){
    ...
    if( (fragmissing = fopen( filename, "rb")) == NULL){
    ...
    /* FILE *ptr; // chris lines for writing in binary (BINARY) */
    /* char outputName[12]; */
    /* sprintf(outputName, "BinaryMissing/mis%d.bin", countingFiles+1); (BINARY) */
    /* countingFiles++; (BINARY) */
    /* ptr=fopen(outputName,"wb"); (BINARY) */
    
    while(fscanf(fragmissing, "%d%lf%lf", &r.m1, &r.m2, &r.m3) != EOF){ // original line
    //while (fread(&r, sizeof(struct rec), 1, fragmissing) == 1) { // chris's loop for reading binary
      (*misseq)[j][i] = r.m1;
      (*misstart)[j][i] = r.m2;
      (*misend)[j][i] = r.m3;
      //printf("missing data: %d\t%e\t%e\n", r.m1, r.m2, r.m3);
      ++i;
      (*missingdata)[j]+=1;

      //fwrite(&r, sizeof(struct rec), 1, ptr); // writing to chris's binary output file (BINARY)
    ...
    //fclose(ptr); // closing chris's binary output file (BINARY) 
    //fprintf(stderr, "fragment %d contains %d missing\n", j, (*missingdata)[j]);
    fclose(fragmissing); // chris is trying to close the missing data files after reading them, here 01112018
                         // using "lsof | wc -l", I confirmed this totally closes each file right after it's read
                         // I also confirmed that the output is exactly the same using the same "-seeds".
                         // this program is total garbage and I wonder if we can even trust it.
...
argcheck( int arg, int argc, char *argv[] )
{
  //fprintf(stderr, "arg: %d\targv[arg]: %s\n", arg, argv[arg]);
         ...
         if( ((arg >= argc ) || ( argv[arg][0] == '-')) && argv[arg][1] != 's' ) {
...
int get_distro_parms(int arg, int argc, char** argv, char distr){
  ...
  while( (++cnt + arg) < argc ){
    ...
    //cnt++;
    // cnt is an int used for the loop, only
    if(cnt == 0 && arg > 0){
      ...
      // this is a normal part of the loop, too
...
int dist_argcheck( int arg, int argc, char* argv[] ){
  ...
  if( (arg < argc) ){
	...
	if(argv[arg][1] == distros[i]){
      ...
      //fprintf(stderr, "distribution is: %s %c\n", argv[arg], distr);
...
int usage3(){
  ...
  fprintf(stderr, "-C <event_number> : Conditional uniform. This draws from U(0,1) and multiplies by the specified event time\n");
  fprintf(stderr, "-Z <min> <max> <event_number>: this takes a min and max AND an event number, and makes sure this parameter does not go smaller than the specified event, currently uses uniform() function\n");
  fprintf(stderr, "-S <event_number> : same time as another event\n");
  fprintf(stderr, "-A <min> <max> <currentEventNumber> : log uniform range of pop size change for calculating exponential growth rate\n\tNOTE: (1) the time for this event must include the -C prior flag,\n\t(2) this event must immediately follow an end of growth command, e.g. -eG -U 100 200 0 (note the zero),\n\t(3) the pop size change range must be between 0 and 1 (think backwards in time shrinking population), and\n\t (4) the current event time must be specified.\n");
  fprintf(stderr, "-B <min> <max> <event> : used at time 0 only***. log uniform range of pop size change for calculating exponential growth rate, specify the event number you want to be the growth time\n");
}
...
double logTransUniform(double a, double b){
  double ran1();
  double r = ran1();
  assert(a<b);
  assert(a > 0 && b > 0);
  double c = log( (b - a)*r + a );
  double rescaled = ( (c-log(a))/(log(b)-log(a)) );
  return rescaled;
}
...
double get_param(int arg, char distr){
  ...
  case 'R':
    ...
    break;
  case 'C':
    priorLimit = param_distr_values[arg][0];
    eventNum = param_distr_values[arg][1] - 1; // minus 1 because event time index 0. (this is reading the number you specifiy after -C )
    // make sure that the specified event came before the current event (or else, we don't have a value for it)
    if( event <= eventNum ){
      printf("\n ERROR: \n");
      printf(" You have asked for a conditional uniform prior (-C) but \n");
      printf(" specified a conditional event <= the current event. \n");
      printf(" The conditional event needs to come before the current event in your command. \n\n");
    }
    // could alter this at some point to allow user-specified prior min
    //return (uniform_ab(0 , 1) * timeevents[eventNum]); // use this return for U(0,specifiedEventTime)
    return uniform_ab(priorLimit , timeevents[eventNum]); // use this to return U(specifiedPriorMin, specifiedEvent)
    //return loguniform(timeevents[eventNum], priorLimit); // use this to return R(specifiedEvent, specifiedPriorLimit)
    break;
  case 'D':
    // this prior distribution flag takes two arguments: the first is the event number which represents the lower limit,
    // the second is the upper limit event time
    eventNum = param_distr_values[arg][0] - 1;
    return uniform_ab( timeevents[eventNum], param_distr_values[arg][1] );
    break;
  case 'Y':
    return ( (logTransUniform(param_distr_values[arg][0], param_distr_values[arg][1])*(param_distr_values[arg][1]-param_distr_values[arg][0]))+param_distr_values[arg][0]);
    break;
  case 'Z':
    priorLimit1 = param_distr_values[arg][0];
    priorLimit2 = param_distr_values[arg][1];
    eventNum = param_distr_values[arg][2] - 1;
    if (priorLimit1 > timeevents[eventNum])
      {
	return uniform_ab(priorLimit1, priorLimit2);
      }
    else
      {
	return uniform_ab(timeevents[eventNum], priorLimit2);
      }
    break;
  case 'S':
    //  I simply want to make this event time as another event 
    eventNum = param_distr_values[arg][0] - 1; // minus 1 because index 0 
    return timeevents[eventNum];
  case 'A':
    // NOTE: the growth event needs to immediately follow an end in growth event, e.g. -eG -U 100 200 0 (note the zero)!!!!
    // NOTE: the growth event with the -A flag needs to have a time specified with "-C" in order for this to work!!!!!!!!!!
    // NOTE: the specified prior range for "fold change" should be between 0 and 1 (think backwards in time)!!!!!!!!!!!!!!!
    // This will be used with growth rates. 'A' species a uniform* range for the fold population size change achieved via exponential growth.
    // The code will calculate an appropriate exponential growth rate to achieve the specified fold change.
    // e.g. if the starting (contemporary) pop size is 1,000,000, and you want a 2-fold change, then the program
    // will calculate an appropriate exponential growth rate according to the growth start and end event times (two events specified).
    // Two events are specified to acheive this. E.g. -eg -U 5000 10000 1 0 -eg -C 1 1 -U 2 5
    //foldChange = loguniform( param_distr_values[arg][0], param_distr_values[arg][1] );
    foldChange = uniform_ab( param_distr_values[arg][0], param_distr_values[arg][1] );  
    //specifiedTime = param_distr_values[arg-3][0] - 1; // this indexing should* work with -C flag for event time, but is totally not robust
    ct1 = (int)param_distr_values[arg][2]; // converting to int
    event1 = timeevents[ct1 -1];
    ct2 = (int)param_distr_values[arg][3]; // converting to int 
    event2 = timeevents[ct2 -1];
    timeElapsed = event1 - event2;
    // to achieve this fold change, the equation is (Hudson, 2007 manual): foldChange = exp( -alpha * time )
    // we have the change, and time. Now, alpha = -( log(foldChange) / time )
    alfa = -(log(foldChange) / timeElapsed);
    return alfa;
    break;
  case 'B':
    // used with a time 0 growth event, specified after another event time, to specify growth size
    //foldChange = loguniform( param_distr_values[arg][0], param_distr_values[arg][1] );
    foldChange = uniform_ab( param_distr_values[arg][0], param_distr_values[arg][1] );
    specifiedTime = (int)param_distr_values[arg][2] - 1;
    timeElapsed = timeevents[specifiedTime];
    alfa = -(log(foldChange) / timeElapsed);
    return alfa;
    break;
  default:
...
int check_conditional_timeevents(int nof_resampling, int** conditional_timeevents, double* timeevents){
  ...
  for(i=0; i< nof_resampling; i++){
    ...
    for(j=2; j<=nofev; j++){
      ...
      // chris is commenting the below line and replacing with the global values conditional line to switch resampling to pop sizes
      //if( timeevents[ev2-1] <= timeevents[ev1-1])
      if( (global_values[ev2] / global_values[ev1]) < 2 )
	return i;
    }
  }
  return -1;
...
/*       strcmp(arg, "--frag-end") && */
      ...
      strcmp(arg, "-ma") &&
      strcmp(arg, "-em") &&
      strcmp(arg, "-s12") && // chris's additions
      strcmp(arg, "-s21") &&
      strcmp(arg, "-s13") &&
      strcmp(arg, "-s31") &&
      strcmp(arg, "-s14") &&
