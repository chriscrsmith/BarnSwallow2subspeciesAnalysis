...
# V2: this version is different from the previous, because in the previous version
# when a sample had missing data at a site, AND the reference had "N" at the site, it outputted N in the fasta,
# which caused the site to be skipped entirely in downstream analyses,
# occassionally (rarely) underestimating the number of seg sites.
# In this version, I will output non-N alleles, having kept track of the common alleles,
# so the fasta should contain zero Ns. 
# The msABC missing data functionality subsequently assigns missing data to these sites.
# The locfile and missing data files sometimes- but usually not- differ in length, but more importantly
# the observed summary statistics will be calculated differently with this script.

import sys, numpy

sys.stderr.write("\n\n\WARNING: For GBS data, there is a separate version of this scriptn\n")
...
sys.stderr.write("\n\n" +  
                 ...
                 "\n    also, currently, my script converts multiple alternate alleles to bi-allelic"
                 "\n\n")
...
minDistanceBetweenLoci = int(sys.argv[11]) # loci are simulated entirely independently, so need to ensure the observed loci are independent
outputType = sys.argv[12] # what type of output. Options: "locusLengths", "fasta", "locfile", "missing", "vcf"

# variable initialization
minSnpQUAL = 0 # minimum QUAL in .vcf to include a snp
locusCounter = 0 # this is only used if outputting fasta
        ...
        for line in vcfFile:
            ...
            if newline[0] == scaf:
                if float(newline[5]) >= minSnpQUAL:
                    # here, checking for multiple alts, and then saving the allele frequencies 
                    if len(newline[4]) > 1:
                        ...
                        AC3 = alleleCollection.count("3")
                        AFs = [AC0,AC1,AC2,AC3]
                        newline[2] = AFs # this is storing extra AF information in the VCF info where there was a blank column 
...
def checkRead(r):
    ...
    if qual == 60: # only considering top quality reads
        ...
        if bits[1] == 1 and bits[2] == 0: # there don't appear to be any obscure bit flags in our reads
...
def genotypeMultipleAlts(PLfieldInfo, r, a, calledGenotype):
    # general strategy, here: I want to deal with only two alleles, so if they have the third, I'll just pretend it's not there.
    # "the order in PL is the same as GL, which follows AA,AB,BB,AC,BC,CC, for tri-allelic sites."
    # I will look at the PL fields. AA,AB,BB for bi-allelic 
    #                               AA,AB,BB,AC,BC,CC for tri-allelic
    # following pattern,            AA,AB,BB,AC,BC,CC,AD,BD,CD,DD for four alleles (I think)
...
paralogThresholds = {}
with open(paralogThresholdPath, "r") as infile:   
    for line in infile:                           
        numSam, thresh = line.strip().split()[:]  
        paralogThresholds[numSam] = float(thresh) 
...
for scaffold in scaffoldOrder:
    if scaffold in goodScaffolds:
        ...
        alignmentMatrix = [] # these two matrices will have rows as long as the scaffold, but indexed so that [0] is position 1 in the sam and vcf
        coverageMatrix = []  
                        ...
                        if (refPosition+p) <= scaffoldLength: # if the position doesn't go beyond the length of the scaffold
                            ...
                            if alignmentMatrix[sample][refPosition+p-1] == "NN": # if we haven't assigned a genotype yet, then genotype it
                                ...
                                if str(refPosition+p) in snps:
                                    ...
                                    ref, alt = snpData[3].upper(), snpData[4].upper()
        ...
        goodRegions = [] 
        nucs = ["A","T","C","G"]
        alleleMatrix = [] # keeping track of the alleles common to the samples (not necessarily the reference allele)
        for position in range(scaffoldLength):
            alleleMatrix.append([])
        for position in range(scaffoldLength): 
            popCounts = [0]*numPops 
            for sample in range(numSamples):
                if coverageMatrix[sample][position] >= minReadDepthPerSample:
                    ...
                    for allele in alignmentMatrix[sample][position]: # for each allele in this sample, check if it's in the matrix of common alleles
                        if allele not in alleleMatrix[position]:
                            if allele != "N":
                                alleleMatrix[position].append(allele)
            outputIt = True # default setting
            for p in range(numPops):
                if popCounts[p] < minSamplesEachPop:
                    outputIt = False
            if outputIt == True and reference[scaffold][position] in nucs: # the ref contains Ns and lower case bases which represent masking
                if newRegion == True:
        ...
        while scanPosition < scaffoldLength and regionIndex < numRegions: 
            ...
            if startPos >= scanPosition: # if the region is spaced far enough apart from the previous region
                ...
                if regionLength >= minimumLocusLength: # this region is long enough to output
                    ...
                    # check heterozygosity in all samples
                    paralog = False
                    countSnps = 0
                            ...
                            for sample in range(9, len(snps[str(position+1)])):
                                ...
                                if dp >= minReadDepthPerSample:
                                    ...
                                    if geno == "0/1":
                                        ...
                                        countHeteros += 1
                                    elif geno == "0/3" or geno == "1/3" or geno == "2/3": 
                                        paralog = True
                            if countCovered >= (minSamplesEachPop*numPops): 
                    ...
                    scanPosition = newEnd + minDistanceBetweenLoci + 1 
                ...
                for pop in pops:
                    for sample in reversePopMap[pop]:
                        ...
                        for position in range(startPos, endPos+1):
                            ...
                            commonAlleles = alleleMatrix[position]
                            if cov >= minReadDepthPerSample:
                                nucs = alignmentMatrix[sampleInd][position]
                                if "N" not in nucs: # no missing data
                                    seq1[posCounter] = nucs[0] # assigning a random allele from this sample to each sequence, because we're not phasing
                                    seq2[posCounter] = nucs[1]
                                    allMissingData = False
                                elif nucs == "NN": # missing data. This might occur if N in reference.
                                    if outputType == "fasta":
                                        if len(commonAlleles) > 0:
                                            seq1[posCounter] = commonAlleles[0] # assign the most common allele as a placeholder
                                            seq2[posCounter] = commonAlleles[0]
                                        else:
                                            pass 
                                    elif outputType == "missing":
                                        pass 
                                else: # one allele is N, not both (this originated from there being multiple alternate alleles)
                                    nucs = nucs.replace("N", "")
                                    seq1[posCounter] = nucs[0]
                                    allMissingData = False
                                    if outputType == "fasta":
                                        seq2[posCounter] = commonAlleles[0] # assign the most common allele as a placeholder
                                    elif outputType == "missing":
                                        pass # leaving as N
                            else:
                                if outputType == "fasta":
                                    if len(commonAlleles) > 0:
                                        seq1[posCounter] = commonAlleles[0]
                                        seq2[posCounter] = commonAlleles[0] # assign the most common allele as a placeholder
                                    else:
                                        sys.stderr.write(scaffold + "," + str(locus[0]) + "," + str(locus[1]) + "," + str(posCounter) + ", all Ns!\n" )
                                        pass 
                                elif outputType == "missing":
