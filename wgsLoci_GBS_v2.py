...
sys.stderr.write("\n\n" +  
                 ...
                 "\n    also, currently script converts multiple alternate alleles to bi-allelic" +
                 "\n\n")
...
minimumLocusLength = int(sys.argv[9]) # the minimum locus length
...
def genotypeMultipleAlts(PLfieldInfo, r, a, calledGenotype):
    ...
    # following pattern,            AA,AB,BB,AC,BC,CC,AD,BD,CD,DD for four alleles 
                        ...
                        for position in range(startPos, endPos+1):
                            ...
                            if cov >= minReadDepthPerSample:                         
                                ...
                                if "N" not in nucs: # no missing data
                                    seq1[posCounter] = nucs[0] # assigning a random allele to each sequence, because we're not phasing
                                    seq2[posCounter] = nucs[1]
                                    allMissingData = False
                                elif nucs == "NN": # missing data
                                    if outputType == "fasta":
